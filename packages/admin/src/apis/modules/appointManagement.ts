...
export function listCount<T>(): Promise<RequestResult<T>> {
  return request.get('/bookGood/count');
}
...
export async function edit<T>(data: { id: string }): Promise<RequestResult<T>> {
